...
import { requestLoginGoogle,requestLoginNaver,requestLogin,requestSignup } from '../../lib/request';
  ...
  const [repeatPassword, setRepeatPassword] = useState('');
    ...
    const handleSubmit = (e) => {
        ...
        if (mode === 'signin') {
          ...
          // 회원가입 로직
          requestSignup( { email, password, repeatPassword })
        }
      };
...
return (
    ...
    <div>
        ...
        <li onClick={() => handleModeChange('signin')} style={{ color: mode === 'signin' ? '#0f132a' : 'rgba(15, 19, 42, 0.6)' }}>
          로그인
        </li>
        &emsp;
        <li onClick={() => handleModeChange('signup')} style={{ color: mode === 'signup' ? '#0f132a' : 'rgba(15, 19, 42, 0.6)' }}>
          회원가입
        </li>
    </div>
      ...
      />
        ...
        <SigninButton onClick={handleSubmit}>{isSignup ? '회원가입' : '로그인'}</SigninButton>
      </Form>
      ...
      <GoogleButton  onClick={()=>requestLoginGoogle()}>      <FaGoogle className="icon" size={30}/>Sign in with Google</GoogleButton>
      <NaverButton onClick={()=>requestLoginNaver()}>      <SiNaver className="icon" size={30}/>Sign in with Naver</NaverButton>
    </Container>
  );
};
